...
# 3. Initialize extensions using the app
db.init_app(app)
...
# ---------------------------------------------------------------------------
# CORE REGISTRY: Map URLs to Models
# ---------------------------------------------------------------------------
MODEL_REGISTRY = {
    "teams": Team,
    "drivers": Driver,
...
# ---------------------------------------------------------------------------
# AUTHORIZATION HELPER & DECORATORS
# ---------------------------------------------------------------------------
def role_required(*roles):
    """Restricts route access based on user role (Admin, Editor, Viewer)."""
    def decorator(f):
        @wraps(f)
...
def cast_value(column, value):
    """Dynamically converts HTML string inputs to the correct Python type based on DB schema."""
...
def get_related_data(model):
    """Detects Foreign Keys and fetches related records for UI dropdowns."""
    related_data = {}
...
def get_model_relationships(model):
    """Inspects SQLAlchemy relationships on the model to dynamically discover join targets."""
...
def add_record(entity):
    ...
    if request.method == "POST":
        ...
        # Set unchecked HTML checkboxes explicitly to False
        for col in columns:
...
def dynamic_join_view(primary_entity=None):
    ...
    return render_template(
        "dynamic_join.html",
        entities=MODEL_REGISTRY.keys(),
        primary_entity=primary_entity,
        relationships=relationships,
        selected_join_keys=active_relationships,
        headers=headers,
        rows=rows
    )

# ---------------------------------------------------------------------------
# Entry point & Initial Database Seed
# ---------------------------------------------------------------------------
